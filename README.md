...
Many ELM327 clones answer only the first PID of a combined request, so
through an ELM327 the client asks for one PID at a time. With one such clone
(an "ELM327 v1.5" on Bluetooth), a Mazda Demio's gear indicator went dark for
as long as the adapter was plugged in and switched on, so unplug adapters when
they are not in use.
